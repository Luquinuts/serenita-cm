# Content Calendars Specification

## Purpose

Calendars are timeless containers for content items — not tied to a specific
month or year. Only items carry dates via `scheduled_date`. Calendars are
visible across all period filters; the grid filters items independently.
